...
// Load backend/.env first; fall back to root .env (monorepo unified file)
const backendEnv = path.join(__dirname, '../../.env');
const rootEnv = path.join(__dirname, '../../../.env');
dotenv.config({ path: backendEnv });
dotenv.config({ path: rootEnv, override: false }); // don't override already-set vars
...
export const config = {
  ...
  upload: {
    ...
    allowedFileTypes: (process.env.ALLOWED_FILE_TYPES || 'image/jpeg,image/png,image/jpg,image/gif').split(','),
  },
};
